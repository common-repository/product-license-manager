<?php
class plm_Licenses_Table extends PLM_List_Table {
    ...
    function column_id($item){
        $row_id = $item['id'];
        $actions = array(
            'edit' => sprintf('<a href="admin.php?page=plmlicense&edit_record=%s">Edit</a>', $row_id),
            'delete' => sprintf('<a href="admin.php?page=plmmanager&action=delete_license&id=%s" onclick="return confirm(\'Are you sure you want to delete this record?\')">Delete</a>',$row_id),
        );
    ...
    function process_bulk_action() {
        if('delete'===$this->current_action()) 
        {
            ...
            if(!isset($_REQUEST['item'])){
                $error_msg = '<p>'.__('Error - Please select some records using the checkboxes', 'product-license-manager').'</p>';
        	...
        	$records_to_delete = $_GET[$nvp_key];
                global $wpdb;
                $record_table_name = PLM_TBL_LICENSE_KEYS;//The table name for the records	
    ...
    function prepare_items() {
        ...
        if (isset($_POST['plm_search'])) {
            $search_term = trim(sanitize_text_field ($_POST['plm_search']));
